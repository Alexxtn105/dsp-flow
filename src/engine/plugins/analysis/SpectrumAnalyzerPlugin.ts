...
interface SpectrumAnalyzerState {
    buffer: Float32Array;
    pointer: number;
    avgSpectrum: Float32Array | null;
}

const SpectrumAnalyzerPlugin = {
    ...
    processor: {
        states: new Map<string, SpectrumAnalyzerState>(),
        ...
        process(inputs: (Float32Array | null)[], params: Record<string, unknown>, chunkSize: number, nodeId: string): Float32Array {
            ...
            const avgFrames = Math.max(1, (params.averaging || 1) as number);
...
export default SpectrumAnalyzerPlugin satisfies PluginDefinition;
