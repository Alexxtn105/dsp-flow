...
interface RefSineGeneratorState {
    currentPhase: number;
}
...
export default {
    ...
    processor: {
        states: new Map<string, RefSineGeneratorState>(),
        ...
        process(inputs: (Float32Array | null)[], params: Record<string, unknown>, chunkSize: number, nodeId: string): Float32Array {
            ...
            for (let i = 0; i < chunkSize; i++) {
                ...
                output[i * 2] = (amplitude as number) * Math.sin(phase);
                output[i * 2 + 1] = (amplitude as number) * Math.cos(phase);
