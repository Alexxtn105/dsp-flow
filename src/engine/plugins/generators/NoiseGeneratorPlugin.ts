...
export default {
    ...
    processor: {
        states: new Map<string, NoiseGeneratorState>(),
        ...
        process(inputs: (Float32Array | null)[], params: Record<string, unknown>, chunkSize: number, nodeId: string): Float32Array {
            ...
            for (let i = 0; i < chunkSize; i++) {
                ...
                if (noiseType === 'white') {
                    output[i] = (amplitude as number) * white;
                    ...
                    output[i] = (amplitude as number) * pink * 0.11; // нормализация ~±1
                    ...
                    if (state.brownLast < -1) state.brownLast = -1;
                    output[i] = (amplitude as number) * state.brownLast;
                }
            }
