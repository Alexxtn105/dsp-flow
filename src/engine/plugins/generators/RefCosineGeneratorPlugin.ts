...
interface RefCosineGeneratorState {
    currentPhase: number;
}
...
export default {
    ...
    group: 'generators',
    signals: { input: 'real', output: 'complex', inputsCount: 2, inputLabels: ['Частота (Гц)', 'Фаза (рад)'] } as const,
    defaultParams: {
        frequency: 1000,
        amplitude: 1.0,
        phase: 0,
    ...
    processor: {
        states: new Map<string, RefCosineGeneratorState>(),
        ...
        process(inputs: (Float32Array | null)[], params: Record<string, unknown>, chunkSize: number, nodeId: string): Float32Array {
            ...
            const basePhaseOffset = ((params.phase ?? 0) as number) * (Math.PI / 180);
            ...
            for (let i = 0; i < chunkSize; i++) {
                const freq = freqInput ? freqInput[i] : baseFrequency as number;
                const phaseOffset = phaseInput ? phaseInput[i] : basePhaseOffset;
                const phaseIncrement = (2 * Math.PI * (freq as number)) / (sampleRate as number);

                const phase = state.currentPhase + phaseOffset;
                // I = cos, Q = -sin (аналитический сигнал)
                output[i * 2] = (amplitude as number) * Math.cos(phase);
                output[i * 2 + 1] = -(amplitude as number) * Math.sin(phase);
