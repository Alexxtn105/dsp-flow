...
export default {
    ...
    processor: {
        states: new Map<string, GoertzelState>(),
        ...
        process(inputs: (Float32Array | null)[], params: Record<string, unknown>, chunkSize: number, nodeId: string): Float32Array {
            ...
            const targetFreq = Math.min((params.targetFrequency as number) ?? 1000, sampleRate / 2);
            const N = (params.N as number) ?? 256;
