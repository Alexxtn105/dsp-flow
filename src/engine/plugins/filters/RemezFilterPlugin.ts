...
function createRemezProcessor() {
    return {
        states: new Map<string, RemezState>(),
        clearStates() { this.states.clear(); },

        init(nodeId: string, params: Record<string, unknown>, sampleRate: number) {
            let order = (params.order as number) || 101;
            if (order % 2 === 0) order += 1; // нечётный для Type I линейной фазы
            const lowCutoff = (params.lowCutoff as number) || 1000;
            const highCutoff = (params.highCutoff as number) || 3000;
        ...
        process(inputs: (Float32Array | null)[], params: Record<string, unknown>, chunkSize: number, nodeId: string): Float32Array {
            ...
            if (!state) {
                ...
                const currentLowCutoff = (params.lowCutoff as number) || 1000;
                const currentHighCutoff = (params.highCutoff as number) || 3000;
                const currentOrder = (params.order as number) || 64;
