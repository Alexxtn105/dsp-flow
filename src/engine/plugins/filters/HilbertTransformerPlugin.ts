...
export default {
    ...
    group: 'filters',
    signals: { input: 'real', output: 'complex' } as const,
    defaultParams: {
        order: 64,
        phaseShift: 90,
    },
    processor: {
        states: new Map<string, HilbertState>(),
        clearStates() { this.states.clear(); },

        init(nodeId: string, params: Record<string, unknown>) {
            const order = (params.order as number) ?? 64;
            ...
            this.states.set(nodeId, {
                ...
                N,
                delay: M,
                _order: (params.order as number) ?? 64
            });
        },
