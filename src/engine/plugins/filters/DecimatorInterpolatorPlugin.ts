...
export default {
    ...
    processor: {
        states: new Map<string, DecimatorState>(),
        ...
        process(inputs: (Float32Array | null)[], params: Record<string, unknown>, chunkSize: number, nodeId: string): Float32Array {
            ...
            const factor = Math.max(1, Math.min(16, Math.round((params.factor as number) ?? 2)));
            const filterEnabled = (params.filterEnabled as boolean) ?? true;
         ...
         */
        _decimate(input: Float32Array, factor: number, filterEnabled: boolean, state: DecimatorState, chunkSize: number): Float32Array {
            let filtered: Float32Array = input;
            if (filterEnabled) {
         ...
         */
        _interpolate(input: Float32Array, factor: number, filterEnabled: boolean, state: DecimatorState, chunkSize: number): Float32Array {
            // Вставка нулей
            ...
            let filtered: Float32Array = expanded;
            if (filterEnabled) {
         ...
         */
        _lowpassFilter(input: Float32Array, cutoffNorm: number, state: DecimatorState): Float32Array {
