 ...
 *
 * Почему комплексные сигналы:
 *   Цифровая модуляция кодирует информацию в двух компонентах — I (синфазная)
 *   и Q (квадратурная). TimingRecovery работает с обеими одновременно, потому что
 *   момент стробирования должен быть одинаковым для I и Q. На выходе — по одному
 *   комплексному отсчёту (I+jQ) на символ в оптимальный момент времени.
 */
import type { PluginDefinition } from '../../types';

interface TimingRecoveryState {
    counter: number;
    prevI: number;
    prevQ: number;
    prevMidI: number;
    prevMidQ: number;
    loopIntegrator: number;
    kp: number;
    ki: number;
    period: number;
    lastKey: string;
...
const TimingRecoveryPlugin = {
    ...
    processor: {
        states: new Map<string, TimingRecoveryState>(),
        ...
        process(inputs: (Float32Array | null)[], params: Record<string, unknown>, chunkSize: number, nodeId: string): Float32Array {
            ...
            const Bn = (params.loopBandwidth ?? 0.01) as number;
...
export default TimingRecoveryPlugin satisfies PluginDefinition;
