...
interface FrequencyDiscriminatorState {
    prevPhase: number;
}

const FrequencyDiscriminatorPlugin = {
    ...
    processor: {
        states: new Map<string, FrequencyDiscriminatorState>(),
        ...
        process(inputs: (Float32Array | null)[], params: Record<string, unknown>, chunkSize: number, nodeId: string): Float32Array {
            ...
            const outputMode = (params.outputMode ?? 'deviation') as string;
...
export default FrequencyDiscriminatorPlugin satisfies PluginDefinition;
