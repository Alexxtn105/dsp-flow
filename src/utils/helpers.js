...
export const getDefaultParams = (blockType) => {
    return registry.getDefaultParams(blockType);
};
...
export const getBlockIcon = (blockType) => {
    return registry.getIcon(blockType);
};
...
export const getBlockSignalConfig = (blockType) => {
    return registry.getSignalConfig(blockType);
};
...
export const areSignalsCompatible = (sourceType, targetType) => {
    // null означает отсутствие порта (генератор без входа или sink без выхода)
    // Соединение невозможно, если у одной из сторон нет порта
